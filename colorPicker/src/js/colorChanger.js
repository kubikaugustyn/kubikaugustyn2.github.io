...
function colourNameToHex(colour) {
    ...
    if (typeof ntcColours[colour] != 'undefined')
        return "#" + ntcColours[colour];
...
function reload() {
    // Step 1: Calculate all missing values
    if (values.name) {
        values.Hex = colourNameToHex(values.name)
        values.HSL = HexToHSL(values.Hex)
        values.RGB = HexToRGB(values.Hex)
    } else if (values.RGB) {
        values.name = ""
        values.Hex = RGBToHex(values.RGB)
        values.HSL = RGBToHSL(values.RGB[0], values.RGB[1], values.RGB[2])
    } else if (values.Hex) {
        values.name = ""
        values.HSL = HexToHSL(values.Hex)
        values.RGB = HexToRGB(values.Hex)
        ...
        values.name = ""
        values.Hex = HSLToHex(values.HSL)
        values.RGB = HSLToRGB(values.HSL)
    }

    // Step 2: Put all the values to the inputs
    setValue('colorName', values.name)
    setValue('R', values.RGB[0])
    setValue('G', values.RGB[1])
    setValue('B', values.RGB[2])
    setValue('Hex', values.Hex.replace("#", ""))
    setValue('H', values.HSL[0])
    setValue('S', values.HSL[1])
    setValue('L', values.HSL[2])
    // Step 3: Set color of the div
    document.getElementById("pickedColor").style.backgroundColor = "rgb(" + values.RGB.join(", ") + ")"
}
