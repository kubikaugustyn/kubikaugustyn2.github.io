let DynmapProjection = L.Class.extend({
...
let DynmapLayerControl = L.Control.Layers.extend({
...
let DynmapTileLayer = L.TileLayer.extend({
    _currentzoom: undefined,
    getProjection: function () {
        return this.projection;
    },
    onTileUpdated: function (tile, tileName) {
        let src = this.dynmap.getTileUrl(tileName);
        tile.attr('src', src);
        tile.show();
    },
    ...
    getTileUrl: function (tilePoint, zoom) {
        let tileName = this.getTileName(tilePoint, zoom);
    ...
    updateNamedTile: function (name) {
        let tile = this._namedTiles[name];
    ...
    _addTilesFromCenterOut: function (bounds) {
        ...
        for (let k = 0, len = this._tilesToLoad; k < len; k++) {
    ...
    _addTile: function (tilePoint, container) {
        let tilePos = this._getTilePos(tilePoint),
    ...
    _nextLoadTile: function () {
        ...
        let next = this._loadQueue.shift();
        if (!next) {
            return;
        }
    ...
    _updateTileSize: function () {
        ...
        if (this._currentzoom !== newzoom) {
            let newTileSize = this.calculateTileSize(newzoom);
    ...
    calculateTileSize: function (zoom) {
        ...
        let zoominlevel = Math.max(0, this.options.mapzoomin - izoom);
    ...
    getTileInfo: function (tilePoint, zoom) {
        ...
        let x = scale * tilePoint.x;
        let y = scale * tilePoint.y;
        return {
...
function splitArgs(s) {
    ...
    $.each(arguments, function (argumentIndex, argument) {
        ...
        obj[argument] = r[argumentIndex - 1];
    });
    return obj;
}

function swtch(value, options, defaultOption) {
    return (options[value] || defaultOption || function () {
    })(value);
}
...
function namedReplace(str, obj) {
    ...
    let result = '';
    while (true) {
        let variableBegin = str.indexOf('{', startIndex);
        let variableEnd = str.indexOf('}', variableBegin + 1);
        if (variableBegin < 0 || variableEnd < 0) {
            result += str.substr(startIndex);
            break;
        }
        if (variableBegin < variableEnd) {
            let variableName = str.substring(variableBegin + 1, variableEnd);
